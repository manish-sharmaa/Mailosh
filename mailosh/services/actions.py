"""The five mail actions (design spec §6.3) — archive, delete, report spam,
star, mark read — as JMAP-correct operations over a selection of messages,
plus everything the UI needs to reconcile itself afterwards.
...
def _result(
    ...
    *,
    ...
    """
    email_ids = [c.before.id for c in changes if c.changed]
    prev = {c.before.id: sorted(c.before.mailbox_ids) for c in changes if c.moved}
